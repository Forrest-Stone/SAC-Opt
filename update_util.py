...
The following code is incorrect and should **not** be used as a reference.
...
def update_constraints(
    ...
    for c in constraints.copy():
        logger.log("The following constraint is being updated:")
        logger.log(json.dumps(c, indent=4))

        if c["error"] == "NO":
            logger.log("The constraint is correct, no need to update.")
        ...
        while k > 0:
            try:
                prompt = prompt_constraints_code.format(
                    ...
                    code=json.dumps(c["code"], indent=4),
                )
    ...
    logger.log("\n\n\n\n+++++++++++++++")
    logger.log("The updated constraints codes are as follows:")
    logger.log(json.dumps(coded_constraints, indent=4))
    logger.log("+++++++++++++++")

    return coded_constraints
