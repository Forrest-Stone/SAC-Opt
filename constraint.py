...
Here is a list of parameters that we have extracted from the description:

-----
{params}
-----

And here is a list of variables that we have defined from the description:
...
- Ensure that the constraints are only the ones that are explicitly mentioned in the description.
- Do not generate the variables or any other information in your response.
...
Here is a list of variables that we has defined from the description:
...
And here is a list of constraints that we has extracted from the description:
...
Here is a list of constraints that we has extracted from the description:
    ...
    if match:
        ...
        if score > 3:
            if logger:
                logger.log("---")
                logger.log(
                    f"The confidence score is {score}, which is high enough.")
                logger.log("---")
            ...
            if action.lower().startswith("y"):
                ...
                constraints.remove(c)
                constraints.append(
                    {"Description": new_constraint,
                        "Formulation": None, "Code": None}
                )
                return True, constraints
            else:
            ...
            for c in constraints.copy():
                ...
                while k > 0:
                    ...
                    if logger:
                        ...
                        logger.log(x)
                        logger.log("+--+")
                    valid, res = q[1](desc, x, params, vars,
                                      constraints, c, logger)
                    if valid:
                        constraints = res
                        break
                    else:
